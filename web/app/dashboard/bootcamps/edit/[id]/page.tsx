...
import styles from "./edit-bootcamp.module.css"
...
export default function EditBootcampPage({ params }: { params: { id: string } }) {
  ...
  const [formData, setFormData] = useState<BootcampFormData>({
    ...
    prerequisites: [],
    image: "",
    instructors: [{ name: "", role: "", bio: "", avatar: "" }],
  ...
  useEffect(() => {
    const fetchBootcamp = async () => {
      try {
        ...
        // Mock bootcamp data
        const mockBootcamp: BootcampFormData = {
          title: "React Native Masterclass",
          subtitle: "Become a mobile app developer",
          description:
            "Learn to build cross-platform mobile apps with React Native, the popular framework for building native apps using JavaScript and React.",
          duration: "12 weeks",
          schedule: "Mon-Wed, 7-9 PM EST",
          startDate: "2023-09-15",
          price: "$1,499",
          level: "Intermediate",
          prerequisites: ["JavaScript fundamentals", "Basic React knowledge", "ES6+ experience"],
          image: "https://example.com/react-native.jpg",
          instructors: [
            {
              name: "John Doe",
              role: "Lead Instructor",
              bio: "John has 10+ years of experience in mobile development and has worked with companies like Facebook and Google.",
              avatar: "https://example.com/john.jpg",
            },
            {
              name: "Jane Smith",
              role: "Assistant Instructor",
              bio: "Jane is a senior React Native developer with expertise in app architecture and state management.",
          ...
          curriculum: [
            {
              ...
              topics: ["Setting up your environment", "React Native basics", "Your first app"],
            },
            {
              week: 2,
              title: "Components and Styling",
              topics: ["Core components", "Custom components", "Styling with StyleSheet"],
            },
          ],
          faqs: [
            {
              question: "Do I need prior experience?",
              answer: "Yes, you should have basic knowledge of JavaScript and React before taking this bootcamp.",
            },
            {
              question: "What will I be able to build after this bootcamp?",
              answer:
                "You'll be able to build professional-grade mobile applications for both iOS and Android platforms.",
            },
          ],
        }

        setFormData(mockBootcamp)
        setLoading(false)
      } catch (error) {
  ...
  const handleInstructorChange = (index: number, field: keyof Instructor, value: string) => {
  ...
  const handleAddInstructor = () => {
    setFormData({
      ...formData,
      instructors: [...formData.instructors, { name: "", role: "", bio: "", avatar: "" }],
    })
  }
  ...
  const handleWeekChange = (index: number, field: keyof CurriculumWeek, value: any) => {
  ...
  const handleAddWeek = () => {
    ...
    setFormData({
      ...formData,
      curriculum: [...formData.curriculum, { week: nextWeekNumber, title: "", topics: [""] }],
    })
  }
  ...
  const validateForm = () => {
    ...
    formData.instructors.forEach((instructor, index) => {
      if (!instructor.name.trim()) newErrors[`instructor-${index}-name`] = "Instructor name is required"
    })

    // Validate curriculum
    formData.curriculum.forEach((week, index) => {
      if (!week.title.trim()) newErrors[`week-${index}-title`] = "Week title is required"
    })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      // In a real app, this would make an API call to update the bootcamp
      console.log("Updating bootcamp:", formData)

      // Redirect to bootcamps management page
    ...
    <DashboardLayout>
      <div className={styles.container}>
        <div className={styles.header}>
          <h1>Edit Bootcamp</h1>
          <p>Update your bootcamp details and curriculum.</p>
        </div>
        ...
        <div className={styles.formContainer}>
          <div className={styles.tabs}>
            ...
            <button
              className={`${styles.tabButton} ${activeTab === "instructors" ? styles.activeTab : ""}`}
              onClick={() => setActiveTab("instructors")}
            >
              Instructors
            </button>
            <button
              className={`${styles.tabButton} ${activeTab === "curriculum" ? styles.activeTab : ""}`}
          ...
          <form onSubmit={handleSubmit} className={styles.form}>
            ...
            {activeTab === "basic" && (
              <div className={styles.tabContent}>
                ...
                <p>Update the essential details about your bootcamp.</p>
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.prerequisitesContainer}>
                    <div className={styles.prerequisiteInput}>
                      ...
                      />
                      <button type="button" onClick={handleAddPrerequisite} className={styles.addButton}>
                        Add
                      </button>
                    </div>
                ...
                <div className={styles.navigationButtons}>
                  <button type="button" onClick={() => setActiveTab("instructors")} className={styles.nextButton}>
                    Next: Instructors
                  </button>
                </div>
              </div>
            ...
            {activeTab === "instructors" && (
              <div className={styles.tabContent}>
                <h2>Instructors</h2>
                <p>Update information about the instructors who will be teaching this bootcamp.</p>
                ...
                {formData.instructors.map((instructor, index) => (
                  <div key={index} className={styles.instructorCard}>
                    ...
                    <div className={styles.formRow}>
                      <div className={styles.formGroup}>
                        ...
                        <input
                          ...
                          className={errors[`instructor-${index}-name`] ? styles.inputError : ""}
                        />
                        {errors[`instructor-${index}-name`] && (
                          <div className={styles.errorMessage}>{errors[`instructor-${index}-name`]}</div>
                        )}
                      </div>
                    ...
                    <div className={styles.formGroup}>
                      <label htmlFor={`instructor-${index}-avatar`}>Avatar URL</label>
                      <input
                        type="text"
                        id={`instructor-${index}-avatar`}
                        placeholder="https://example.com/avatar.jpg"
                        value={instructor.avatar}
                        onChange={(e) => handleInstructorChange(index, "avatar", e.target.value)}
                      />
                    </div>
                  </div>
                ))}

                <button type="button" onClick={handleAddInstructor} className={styles.addInstructorButton}>
                  + Add Another Instructor
                </button>

                <div className={styles.navigationButtons}>
                  <button type="button" onClick={() => setActiveTab("basic")} className={styles.backButton}>
                    Back: Basic Info
                  </button>
                  <button type="button" onClick={() => setActiveTab("curriculum")} className={styles.nextButton}>
                    Next: Curriculum
                  </button>
                </div>
              </div>
            ...
            {activeTab === "curriculum" && (
              <div className={styles.tabContent}>
                <h2>Curriculum</h2>
                <p>Update your bootcamp curriculum by week. Add topics for each week.</p>
                ...
                {formData.curriculum.map((week, weekIndex) => (
                  <div key={weekIndex} className={styles.weekCard}>
                    ...
                    <div className={styles.formGroup}>
                      <label htmlFor={`week-${weekIndex}-title`}>Week Title *</label>
                      <input
                        type="text"
                        id={`week-${weekIndex}-title`}
                        value={week.title}
                        onChange={(e) => handleWeekChange(weekIndex, "title", e.target.value)}
                        className={errors[`week-${weekIndex}-title`] ? styles.inputError : ""}
                      />
                      {errors[`week-${weekIndex}-title`] && (
                        <div className={styles.errorMessage}>{errors[`week-${weekIndex}-title`]}</div>
                      )}
                    </div>
                    ...
                    <div className={styles.formGroup}>
                      ...
                      <button type="button" onClick={() => handleAddTopic(weekIndex)} className={styles.addTopicButton}>
                        + Add Topic
                      </button>
                    </div>
                  </div>
                ...
                <button type="button" onClick={handleAddWeek} className={styles.addWeekButton}>
                  + Add Another Week
                </button>

                <div className={styles.navigationButtons}>
                  <button type="button" onClick={() => setActiveTab("instructors")} className={styles.backButton}>
                    Back: Instructors
                  </button>
                  <button type="button" onClick={() => setActiveTab("faqs")} className={styles.nextButton}>
                    Next: FAQs
                  </button>
                </div>
              </div>
            ...
            {activeTab === "faqs" && (
              <div className={styles.tabContent}>
                ...
                <p>Update FAQs to help potential students learn more about your bootcamp.</p>
                  ...
                  <div key={index} className={styles.faqCard}>
                    <div className={styles.faqHeader}>
                      ...
                      {formData.faqs.length > 1 && (
                        <button type="button" onClick={() => handleRemoveFaq(index)} className={styles.removeButton}>
                          Remove
                        </button>
                      )}
                    </div>
                ...
                <button type="button" onClick={handleAddFaq} className={styles.addFaqButton}>
                  + Add Another FAQ
                </button>

                <div className={styles.navigationButtons}>
                  <button type="button" onClick={() => setActiveTab("curriculum")} className={styles.backButton}>
                    Back: Curriculum
                  </button>
